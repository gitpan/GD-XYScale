...
my $some_color = $image->colorAllocate(100,60,220);
   ...
   $image->origin(400,300);
   $image->draw_xyscale(1.5 ,$image->colorAllocate(0,0,255));
   $image->name_xyscale('up','X-Scale','Y-Scale',$red,undef,'show_zoom');

   # The point values (X,Y) needs to be fixed, to display 
   # them corrrectly on the scale.
   # Fix points (X and Y)
   $image->line($image->fixp2o(15, 56),$image->fixp2o(44, 365),$red);
   $image->line($image->fixp2o(15,-56),$image->fixp2o(44,-365),$red);
   ...
   # Usage of aliases
   $image->line($image->fix_point_to_origin(15, 56),
                $image->fix_point_to_origin(44, 365),
                $red);

   $image->line($image->fix_value_x_to_origin(10),  $image->fix_value_y_to_origin(55),
                $image->fix_value_x_to_origin(256), $image->fix_value_y_to_origin(189.56),
                $some_color);
